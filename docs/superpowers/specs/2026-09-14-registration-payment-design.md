...
1. 主流程：OPS 建赛事、配两个价格档（早鸟 + 标准）并开放报名 → FINANCE 建收款账户 → 跑者（测试代码签名 `initData`）两人同单、使用百分比优惠码下单 → 付款页金额与算价一致 → 上传截图 → FINANCE 以 `UNREADABLE` 驳回 → 跑者看到驳回原因并重传 → FINANCE 通过 → 跑者订单显示已确认和参赛凭证。
2. 免单码下单直接显示已确认。
